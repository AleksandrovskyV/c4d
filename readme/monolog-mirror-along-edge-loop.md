...
Текстовой процесс появления "Mirror along Edge Loop"\
...
Понял задачу: вам нужно именно выровнять существующие точки модели, чтобы они встали строго симметрично, не меняя топологию (ID точек) и абсолютно не трогая UV-развертку (без разрезания, удаления половин и создания дубликатов).
...
Чтобы выровнять точки без дублирования:
...
Так как количество и индексы точек абсолютно не изменились, старая развертка идеально сядет на выровненную модель. 
Maxon
...
#
<br>
Я не думаю, что алгоритм вообще ищет пары точек. Гораздо проще строить дерево от одной, что находится на плоскости симметрии
...
Именно благодаря такому гибридному подходу в новых версиях Cinema 4D вы можете выровнять точки деформированной модели с готовой UV-разверткой «в один клик», и программа поймет, какая точка какой соответствует, даже если левая рука персонажа согнута, а правая — опущена.
...
В Cinema 4D исторически отсутствует аналог функции Snap to Symmetry / Symmetrize (с сохранением ID), который есть, например, в Maya или Blender (где можно выбрать сторону-донора и просто выровнять вершины существующего меша, не ломая развертку).
...
#
<br>
Нет условного доктора топологии в версии 2026, который бы вылечил неправильные индексы в 26 версии? Ведь Symmetry Hub работает на стандартных сферах
...
* Как проверить: Выделите все полигоны (Ctrl + A). Если часть из них синяя, кликните правой кнопкой мыши ➔ Align Normals (Выровнять нормали).
...
2. Проверка, что все точки находятся в плоскости. Если нет - предупреждение, что плоскость будет выровнена. Выбор из пунктов (ок) - продолжаем, (отмена) - отмена
...
2. Скрипт должен сам находить плоскость симметрии, а не просто работать относительно 0 координаты. Если центральный луп не находится на одной плоскости - выскакивает предупреждение для пользователя: "выровнять ли все точки относительно плоскости сечения?"
...
1. Не гуглится, хотя идея связать с инструментом R26 - неплохо. Есть плохая побочка, и скорее всего  я просто добавлю это в расширенное описание
...
#
<br>
Мою репу могут удалить, да и новые версии Cinema 4D вызывают большой скепсис. В любом случае это вечная борьба с ветряными мельницами. Но если вдруг - попробуй проанализировать, почему именно [этот код](https://github.com/AleksandrovskyV/c4d/blob/main/vsky.scripts/Mirror%20along%20Edge%20Loop.py) мне кажется «ок», включая его интерфейс
